...
if 'DATABASE_URL' in os.environ:
    ...
    db = PostgresqlDatabase(database=url.path[1:], user=url.username, password=url.password, host=url.hostname, port=url.port, autorollback=True)
else:
...
def clean_db():
    Task.update(freshly_created=False, freshly_completed=False, new_idea=False).where(True).execute()
...
def refresh_db(tasks): 
    ...
    for task in tasks:
        ...
        freshly_created = freshly_created or t.freshly_created
        freshly_completed = (t.status != task['status'] and task['status'] == 'Completed') or t.freshly_completed
        new_idea = (t.status != task['status'] and task['status'] == 'Idea') or t.new_idea

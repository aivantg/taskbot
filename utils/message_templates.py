...
nl = '\n'
basic_task = lambda t: f"{status_emojis.get(t['status']) or ''}*{t['name']}*{status_emojis.get(t['status']) or ''}"
basic_task_with_assignees = lambda t: f"{status_emojis.get(t['status']) or ''}*{t['name'].upper()}*{status_emojis.get(t['status']) or ''}, *Assigned*: {', '.join(t['assignees']) if t['assignees'] else 'No One'}" 
full_task = lambda t: f"{status_emojis.get(t['status']) or ''}*{t['name']}*{status_emojis.get(t['status']) or ''}, *Due*: In {t['days_left']} Days, *Assigned*: {', '.join(t['assignees']) if t['assignees'] else 'No One'}"
overdue_task = lambda t: f"{status_emojis.get(t['status']) or ''}*{t['name']}*{status_emojis.get(t['status']) or ''}, *Due*: {abs(t['days_left'])} Days Ago, *Assigned*: {', '.join(t['assignees']) if t['assignees'] else 'No One'}"

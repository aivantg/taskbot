...
full_task = lambda t: f"{status_emojis.get(t['status']) or ''}*{t['name']}*, Due: _In {t['days_left']} Days_, Assigned: {', '.join(t['assignees']) if t['assignees'] else 'No One'}"
overdue_task = lambda t: f"{status_emojis.get(t['status']) or ''}*{t['name']}*, Due: _{abs(t['days_left'])} Days Ago_, Assigned: {', '.join(t['assignees']) if t['assignees'] else 'No One'}"
